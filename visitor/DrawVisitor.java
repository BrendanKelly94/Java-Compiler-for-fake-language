...
public class DrawVisitor extends ASTVisitor {
    ...
    public void draw(String s, AST t) {
        ...
        //width/2 + progress[depth]*hstep - nCount[depth]*hstep/2;
        int x = width / 2 + t.getXCoor() * hstep / 2;
        int y = depth * vstep;
        ...
        for (int i = 0; i < t.kidCount(); i++) {
            //endx = width / 2 + (t.getXCoor() + i) * hstep / 2 + nodew / 2;
            endx = width / 2 + k.get(i).getXCoor() * hstep / 2 + nodew / 2;
            endy = (depth + 1) * vstep;
