...
public class Parser {
    ...
    public AST rType() throws SyntaxError {
        ...
        } else if (isNextTok(Tokens.Float) || isNextTok(Tokens.ScientificN)) {
            t = new FloatTypeTree();
            scan();
    ...
    public AST rFunHead() throws SyntaxError {
        //formal example: add(int a, int b)
        //actual example: add(3,2)
     ...
     * @exception SyntaxError - thrown for any syntax error
     */
    public AST rFactors() throws SyntaxError {
        AST t;
        ...
        if (isNextTok(Tokens.INTeger)) {  //  -> <int>
            ...
            scan();
            return t;
        } else if (isNextTok(Tokens.FLOAT) || isNextTok(Tokens.SCIentificN)) {  //  -> <float>, <scientificN>
            t = new FloatTree(currentToken);
            scan();
            return t;
        } else if (isNextTok(Tokens.CHAR)) {  //  -> <char>
            t = new CharTree(currentToken);
            scan();
            return t;
        } else if (isNextTok(Tokens.STRING)) {  //  -> <String>
        ...
        if (isNextTok(Tokens.Minus)) {
            t = new NegOpTree(currentToken);
            scan();
            t.addKid(rExpr());
            return t;
            //->!E
        } else if (isNextTok(Tokens.Not)) {
...
class SyntaxError extends Exception {
    ...
    //add more information to this error
    void print() {
